...
public class MyAccount_Page extends BaseClass {
	...
	public static class WishList {
		...
		public static List<WebElement> WishListItemRemove() throws Exception {

			try {
				Log.info("Locating WishListItemRemove elements");
				elementList = Driver.findElements(By.xpath(".//*[@id='qv-drop']//*[@class='remove-pro']"));
               // Utils.highLight(mainElement);
				Log.info("WishListItemRemove button is found on the Home Page");

			} catch (Exception e) {
				Log.error("Exception in Class MyAccount_Page | Method WishListItemRemove");
				Log.error("WishListItemRemove button is not found on the Home Page");
				throw e;
			}
	...
	public static class MyAccount_LeftMenu {
		...
		public static WebElement Profile() throws Exception {
			...
			try {
				...
				mainElement = Driver.findElement(By.xpath("(//a[contains(text(),'Profile')])[2]"));
				Utils.highLight(mainElement);
