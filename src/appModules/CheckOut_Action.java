...
public class CheckOut_Action {
	...
	public static void validateNumberOfDefaultPaymentMode()throws Exception{
		try{
			...
			if(numberOfDefaultModeOfPayment==i){
				System.out.println("Number of Payment Mode is equals to 5");
				Log.info("Number of Payment Mode is equals to 5");
			}
			else{
				BaseClass.errorValidation+="Number of Payment mode is not equals to 5\n";
			}
	...
	public static void Verify_Private_ChecoutPage_ApplyCoupan(int iTestCaseRow) throws Exception {
		...
		totalCartPrice = Checkout_Page.Paymentinfo.TotalPrice().getText();
		...
		try {
			...
			if (!(Cart_Page.CartPageAlert().getText().equals("Voucher applied successfully!"))) {
				BaseClass.errorValidation += "'Voucher applied successfully!' - alert not present on page. \n";
			}
			if (Checkout_Page.Paymentinfo.TotalPrice().getText().equals(totalCartPrice)) {
	...
	public static void DirectCheckoutFromMiniCart(int iTestCaseRow)throws Exception{
		try{
			...
			System.out.println("Direct Checkout from Minicart failed");
			throw new Exception("Direct Checkout from Minicart Failed || Method DirectCheckoutFromMiniCart");
		}
	}
...
public static void ValidateErrorMessageForEmptyDeliveryAddress(int iTestCaseRow) throws Exception{
			...
			try{
				...
				Utils.verifyElement(Checkout_Page.DeliveryAddress.ErrorMessageForEmptyFields());
				Checkout_Page.DeliveryAddress.phone().sendKeys(ExcelUtils.getCellData(iTestCaseRow, Constant.mobileNumber));
			}
			catch(Exception ex){
				BaseClass.errorValidation+=" Please enter your Mobile Number message not displayed \n";
			}
			...
			try{
				...
				BaseClass.errorValidation+="  Please enter City message not displayed. \n";
			}
	...
	public static void RedeemGiftCard_EGV_FCC(int iTestCaseRow) throws Exception {
		...
		try {
			...
			if (paymentMode.equals("GiftCard")) {
				...
				Thread.sleep(5000);
				Utils.verifyElement(Checkout_Page.Paymentinfo.Redeem_Message());
			}
...
//				}
				...
				Log.info("Card number is entered successfully");
				Checkout_Page.Paymentinfo.CreditCard_ExpiryMonth().selectByIndex(3);
				Log.info("Month is selected successfully");
				Checkout_Page.Paymentinfo.CreditCard_ExpiryYear().selectByIndex(3);
			...
			if (paymentMode.equals("DebitCard")) {
				...
				Log.info("Card number is entered successfully");
				Checkout_Page.Paymentinfo.DebitCard_ExpiryMonth().selectByIndex(4);
				Log.info("Month is selected successfully");
				Checkout_Page.Paymentinfo.DebitCard_ExpiryYear().selectByIndex(4);
				Log.info("Year is selected successfully");
				Checkout_Page.Paymentinfo.DebitCard_CVV().sendKeys(cvvNumber);
				Log.info("CVV number is entered successfully");
				Checkout_Page.Paymentinfo.DebitCard_Name().selectByIndex(4);
				Log.info("Card name is selected successfully");
...
public static void PaymentOption_MasterValidation(int iTestCaseRow) throws Exception{
		...
		try {
				...
				Utils.verifyElement(Checkout_Page.Paymentinfo.CreditCard_CardNumber());
				Log.info("Card number is Verified successfully");
				Checkout_Page.Paymentinfo.CreditCard_ExpiryMonth();
				Checkout_Page.Paymentinfo.CreditCard_ExpiryYear();
				
				Utils.verifyElement(Checkout_Page.Paymentinfo.CreditCard_CVV());
				Log.info("CVV number is entered successfully");
				Checkout_Page.Paymentinfo.CreditCard_Name();
			

				Utils.verifyElement(Checkout_Page.Paymentinfo.CreditCard_PlaceOrderBtn());
				Checkout_Page.Paymentinfo.collapsePaymentMode().click();
			}
			catch(Exception ex){
			...
			try{
				...
				try{
					...
					Log.error("Exception in Class PaymentOption_Wallets | Method PaymentOption_MasterValidation");
				...
				Utils.verifyElement(Checkout_Page.Paymentinfo.Wallet_PlaceOrderBtn());
				Checkout_Page.Paymentinfo.collapsePaymentMode().click();
			...
			try{
				...
				Checkout_Page.Paymentinfo.DebitCardOption().click();
				Utils.verifyElement(Checkout_Page.Paymentinfo.DebitCard_CardNumber());
				Checkout_Page.Paymentinfo.DebitCard_ExpiryMonth();
				Checkout_Page.Paymentinfo.DebitCard_ExpiryYear();
				Utils.verifyElement(Checkout_Page.Paymentinfo.DebitCard_CVV());
				Checkout_Page.Paymentinfo.DebitCard_Name();
				Utils.verifyElement(Checkout_Page.Paymentinfo.DebitCard_PlaceOrderBtn());
				Checkout_Page.Paymentinfo.collapsePaymentMode().click();
			}
			catch(Exception ex){
			...
			try{
				Utils.verifyElement(Checkout_Page.Paymentinfo.COD_PaymentOption());
