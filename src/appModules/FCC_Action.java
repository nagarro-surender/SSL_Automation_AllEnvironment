...
//		}
		try {
			...
			FCC_Page.FCC_Form.FCCForm_Title().selectByVisibleText(ExcelUtils.getCellData(iTestCaseRow, Constant.title));
			Log.info("Title is selected");
			FCC_Page.FCC_Form.FCCForm_NameOnCard().sendKeys(ExcelUtils.getCellData(iTestCaseRow, Constant.firstName));
			Log.info("Title is selected");
			Utils.jsClick(FCC_Page.FCC_Form.FCCForm_NameOnCardSame());
			FCC_Page.FCC_Form.FCCForm_DateOfBirth().sendKeys(ExcelUtils.getCellData(iTestCaseRow, Constant.DOB));
			//Thread.sleep(2000);
			Log.info("Dateofbirth is selected");
			FCC_Page.FCC_Form.FCCForm_HomeSTDCode().sendKeys(ExcelUtils.getCellData(iTestCaseRow, Constant.homeSTD));
			Log.info("homestd is selected");
			//Thread.sleep(2000);
			FCC_Page.FCC_Form.FCCForm_HomeLandline().sendKeys(ExcelUtils.getCellData(iTestCaseRow, Constant.homeLandline));
			//Thread.sleep(2000);
			FCC_Page.FCC_Form.FCCForm_OfficeSTDCode().sendKeys(ExcelUtils.getCellData(iTestCaseRow, Constant.officeSTD));
			//Thread.sleep(2000);
			FCC_Page.FCC_Form.FCCForm_OfficeLandline().sendKeys(ExcelUtils.getCellData(iTestCaseRow, Constant.officeLandline));
			//Thread.sleep(2000);
			FCC_Page.FCC_Form.FCCForm_FirstName().sendKeys(ExcelUtils.getCellData(iTestCaseRow, Constant.firstName));
			FCC_Page.FCC_Form.FCCForm_LastName().sendKeys(ExcelUtils.getCellData(iTestCaseRow, Constant.lastName));
			String email=ExcelUtils.getCellData(iTestCaseRow, Constant.emailId);
			Random r=new Random();
			int rInt=r.nextInt(1000);
			email=rInt+email;
			System.out.println(email);
			FCC_Page.FCC_Form.FCCForm_Email().sendKeys(email);
...
//					.sendKeys(ExcelUtils.getCellData(iTestCaseRow, Constant.confirmPassword));
			FCC_Page.FCC_Form.FCCForm_Gender().sendKeys(Keys.SPACE);
...
//		}
	...
	public static void FCC_Verify_AddToBagFCC_GuestUser_ViaFooterLink(int iTestCaseRow) throws Exception {
		...
		try {
			Utils.verifyElement(FCC_Page.AddToBag());
			Thread.sleep(3000);
			Utils.jsClick(FCC_Page.AddToBag());
		} catch (Exception e) {
			Log.error(e.getMessage());
			Log.error("Exception in Class FCC_Action | Method FCC_Verify_AddToBagFCC_GuestUser_ViaHeaderLink");
			throw e;
	...
	public static void FCC_Verify_QuickBuyFCC_ViaMyAccountPage(int iTestCaseRow) throws Exception {
		...
		try {
			Utils.verifyElement(MyAccount_Page.MyAccount_LeftMenu.FirstCitizenProfile());
			Utils.jsClick(MyAccount_Page.MyAccount_LeftMenu.FirstCitizenProfile());
		} catch (Exception e) {
		...
		try {
			Utils.verifyElement(MyAccount_Page.FirstCitizen_Profile.FirstCitizenButton());
			Utils.jsClick(MyAccount_Page.FirstCitizen_Profile.FirstCitizenButton());
		} catch (Exception e) {
			Log.error("Exception in Class FCC_Action | Method FCC_Verify_QuickBuyFCC_ViaMyAccountPage");
			Log.error(e.getMessage());
			throw e;
	...
	public static void FCC_Verify_QuickBuyFCC_GuestUser_ViaHeaderLink(int iTestCaseRow) throws Exception {
		...
		try {
			...
			Utils.jsClick(FCC_Page.QuickBuyGuestUser());
		} catch (Exception e) {
			Log.error(e.getMessage());
	...
	public static void FCC_Verify_Update_FCCProfile_Functionality(int iTestCaseRow) throws Exception {
		...
		try {
			...
			Utils.jsClick(FCC_Page.FCCEditCardProfile());
